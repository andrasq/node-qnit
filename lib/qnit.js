 ...
 *
 * Copyright (C) 2015,2017-2020 Andras Radics
...
module.exports.runTestFunction = runTestFunction;
...
// err came from a try/catch so is a thrown error even if falsy, make it truthy
function makeTruthyError( err) {
    if (err) return err;
    var err2 = new Error('falsy error <' + (typeof err) + ' ' + err + '>');
    err2.stack = err.stack;
    if (!err2.stack) Error.captureStackTrace(err2, makeTruthyError);
    return err2;
}
function runSuite( argv, options, done ) {
    ...
    runFiles(argv, options, stats, function(err) {
        ...
        var errorCount = stats.errors.length + (err && err !== 'stop' ? 1 : 0);
        var summaryLine = sprintf(
            "%d assertions, %d errors, %d tests, %d files, %d skipped (%sms)\n",
            stats.assertionCount, errorCount, stats.testCount, stats.fileCount, stats.skippedCount,
...
function spawnFile( filename, options, stats, done ) {
    ...
    try { child = child_process.fork(require.resolve("./worker.js"), [], {stdio: [process.stdin, process.stdout, process.stderr, 'ipc']}); }
...
function runTest( obj, name, options, setups, teardowns, stats, done ) {
    ...
    function before( cb ) {
        ...
        // mocha compat before() come in an arrays, and all have callbacks
        if (Array.isArray(obj.before)) aflow.applyVisitor(obj.before, function(fn, cb) { fn.call(stats._beforeThis, cb) }, cb);
        else { obj.before.call(stats._beforeThis, cb); if (!obj.before.length) cb() };
    }
    ...
    function after( cb ) {
        ...
        else { obj.after.call(stats._beforeThis, cb); if (!obj.after.length) cb() };
    }
...
function runTestFunction( test, name, options, setups, teardowns, stats, done ) {
    ...
    // bind callback methods so they can be used as functions (in eg setImmediate)
    tester.skip = QUnit.prototype.skip.bind(tester);
    tester.done = QUnit.prototype.done.bind(tester);
    tester.fail = QUnit.prototype.fail.bind(tester);
    ...
    runit(tester, test, setups, teardowns, function(err) {
        ...
        if (options._selftestonly) return done(err);
    ...
    function runit(tester, testFunc, setups, teardowns, callback) {
        ...
        aflow.applyVisitor(
            setups,
            function(setUp, cb) {
                // qunit calls take an optional callback, we have to call cb() if they dont
                // setUp.length ? setUp.call(testContext, cb) : { setUp.call(testContext); cb() }
                setUp.call(testContext, cb);
                if (!setUp.length) cb();
            },
            function(err) {
                ...
                tester._onDone = function whenDone(err) {
                    // run all the teardowns, even after test error (since v0.29)
                    aflow.applyVisitor(
                        teardowns,
                        function(tearDown, cb) {
                            tearDown.call(testContext, cb);
                            if (!tearDown.length) cb();
                            // TODO: should run all teardowns, even if one errors out
                        },
                        function(err2) {
                            // teardown errors are also fatal
                            // TODO: run all teardowns, gather up and report all errors
                            // TODO: do not kill the test suite on teardown error
                            if (err2) throw err2;
                            done(err || err2, testContext);
                        }
                    );
                }

                // trap test errors, send them to _onDone
                // TODO: handle returned Promises
                try { testFunc.call(testContext, tester) }
                catch (err) { tester._onDone(makeTruthyError(err)) }
            }
        );
    }
...
// speed access
QUnit.prototype = toStruct(QUnit.prototype);
