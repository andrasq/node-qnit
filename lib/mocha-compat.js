...
// TODO: if skipped, change enclosed tests to it.skip
...
global.it.skip = function skipIt( name, test ) {
    var nf = getNameFunc(name, test, true);
    testObject._tests.push(
        { name: nf.name, test: function(t) { t.skip() } });
    testObject._count += 1;
};

// many mocha functions take an optional name, and an optional callback
// Convert them to canonical qunit test functions that always take a callback.
function getNameFunc(_name, _fn, needCb) {
    ...
    if (!fn.length && needCb) {
        nf.func = function(cb) {
            var e = fn();
/**
            // compat: convert a Promise to test results
