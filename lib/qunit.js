...
function runTests( argv, options, done ) {
    aflow.applyVisitor(
        argv,
        function visitor(filename, next) {
            ...
            if (stat.isFile()) {
                ...
                aflow.filter(
                    files,
                    function(file, next) { next(null, file.slice(-3) == '.js') },
                    function(err, files) {
                        if (err) return next(err);
                        runTests(files, options, next);
                    }
                );
// FIXME: filter for just .js files
            }
        },
        function(err) {
...
function run( obj, name, options, done ) {
    ...
    aflow.applyVisitor(
        ...
        function(err) {
            ...
            if (err && tester._test) reportit(tester, err, tester._test, function() {
                done(err);
            })
            else done();
        }
