...
function runSuite( argv, options, done ) {
    ...
    function exitListener( ) {
        if (isRunning) {
            process.stdout.write("\nunexpected exit, tests not done (" + (Date.now() - startTime) + "ms)\n");
        }
    };
...
// FIXME: an exception from here returns as another callback to here!
...
function run( obj, name, options, done ) {
    ...
    function reportit( tester, err, test, cb ) {
        ...
        tester.printf(format, ok, tester._name, duration, err);
