    ...
    child.on('message', function responseListener(message) {
        if (!message) {
            ...
            if (message.stats) {
                ...
                stats.fileCount = message.stats.ileCount;
            }
        }
...
function runTest( obj, name, options, setups, teardowns, stats, done ) {
    if (typeof obj === 'function') {
        ...
        // TODO: if (options.grep !== false && !options.grep.test(name)) return done();
        if (options.filter !== false && !name.match(options.filter)) return done();
...
function runTestFunction( test, name, options, setups, teardowns, stats, done ) {
    ...
    function runit(tester, testFunc, setups, teardowns, cb) {
        ...
        aflow.applyVisitor(
            setups,
            function(setUp, cb) { setUp.call(testContext, cb) },
            function(err) {
                if (err) throw err;
                tester._onDone = function whenDone(err) {
                    ...
                    aflow.applyVisitor(
                        teardowns,
                        function(tearDown, cb) { tearDown.call(testContext, cb) },
                        function(err) {
                            cb(err, testContext);
                        }
                    );
                }
                testFunc.call(testContext, tester);
            }
        );
    }
