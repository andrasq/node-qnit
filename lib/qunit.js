...
// TODO: print dots on the screen to show test progress, like phpunit?
        ...
        stats.fileCount += 1;
        runTest(module, "", options, [], [], stats, function(err) {
            done(err);
        });
    }
    catch (err) {
...
function runTest( test, name, options, setups, teardowns, stats, done ) {
    ...
    if (typeof test === 'function') {
        ...
        var timer = setTimeout(function(){
            var err = new Error("test did not call done() within " + testTimeout + " ms");
...
function runTestObject( obj, name, options, setups, teardowns, stats, done ) {
    var specialMethods = ['setUp', 'tearDown', 'before', 'after', 'beforeEach', 'afterEach'];
    var tests = new Array();

    // before/after are run just once
    if (obj.before) tests.push(obj.before);
    for (var i in obj) if (specialMethods.indexOf(i) < 0) tests.push(i);
    if (obj.after) tests.push(obj.after);

    // beforeEach/afterEach/setUp/tearDown are run anew for each test 
    if (obj.setUp) setups.push(obj.setUp);
    if (obj.beforeEach) setups.push(obj.beforeEach);
    if (obj.tearDown) teardowns.unshift(obj.tearDown);
    if (obj.afterEach) teardowns.unshift(obj.afterEach);
